...
The main status packet. The controller returns this in response to `02 55` (and
a longer variant for `02 56`); the component polls it on every update interval,
and it is the source for most of the sensor values.

Offsets are into `data[]`, which includes the `02 55` prefix at 0 and 1. Four
captures are shown side by side so that the fields which vary are visible.

The labels here were worked out from captures alone, so they are less certain
than those in [swg.md](swg.md) and [lights.md](lights.md). Blank rows are fields
